...
There is also special kind of records, `events`, that are generated by plugins themselves, not by data from external source.
...
As explained in [Configuration file terminology](#configuration-file-terminology) section, plugins can be divided into three types, `monitors`, `filters` and `actions`. The `monitors` can only produce new records. `filters` consume records and decide to either output it or not based on it's settings. `actions` only consume `records`, but can produce `events`, for example in case of error.
...
In it's simplest form `chain` includes one monitor, zero or more `filters` and ends with an `action`. However for convenience sake it is possible  to list multiple `monitors` sequentially in one `chain`:
...
When a `monitor` receives `record` as input, it will be passed down the chain unchanged. In the example above records from `rss` monitor will fall through `twitch`, ending in `discord.hook` along with records from `twitch` itself.

***

If plugin in the middle of a `chain` lists multiple entities, all of them will receive records from previous one, and all of them will generate records based on it.
...
In case more than one entity of such plugin lets through the same record, it will effectively be duplicated, coming down the chain as two. To mitigate it either give each entity it's own `chain`, or use `filter.deduplicate` plugin to drop repeating `records` before passing it to the `action`:
...
It is possible for multiple sources to produce duplicate records simple because they monitor the same source, but in this example records from "subscriptions" will be duplicated while passing through two entities of `rss` plugin. Then one of them will be dropped on `filter.deduplicate`.
...
In the following example records from Youtube and Twitch monitors are fed into "multiplexor" entity of `noop` filter, which simple passes all records through unchanged. Output of the "multiplexor" then passed to `discord.hook` "notifications" entity.
