...
<!-- This file is autogenerated as part of avtdl at 2026-06-17 16:07:03.006219 -->
...
* [Monitors:](#monitors)
  ...
  * [get_url - Monitor web page state](#get_url---monitor-web-page-state)
  * [twitcast - Monitor for twitcasting.tv](#twitcast---monitor-for-twitcastingtv)
  * [nicochannel.news - Monitor Nicochannel fanclub NEWS tab](#nicochannelnews---monitor-nicochannel-fanclub-news-tab)
  * [nicochannel - Monitor Nicochannel fanclub](#nicochannel---monitor-nicochannel-fanclub)
...
* [Filters:](#filters)
  ...
  * [filter.format.opaque - Generate record with custom fields](#filterformatopaque---generate-record-with-custom-fields)
* [Actions:](#actions)
  ...
  * [youtube.live - Wait for livestream on Youtube](#youtubelive---wait-for-livestream-on-youtube)
  ...
  * [nicochannel.live - Wait for livestream on Nicochannel](#nicochannellive---wait-for-livestream-on-nicochannel)
...
* `transport`: HTTP transport library to use for making requests. Default value is `aiohttp`.
...
* `only_likes`: user likes are long disabled and this option doesn't work. Default value is `false`.
...
#### Plugin configuration options:
* `login`: rplay login of the account used for monitoring. Required.
* `password`: rplay password of the account used for monitoring. Required.
##### 
* `db_path`: path to the sqlite database file keeping history of old records.
Might specify a path to a directory containing the file (with trailing slash)
or a direct path to the file itself (without a slash). If special value `:memory:` is used,
database is kept in memory and not stored on disk at all, providing a clean database on every startup. Default value is `db/`.
* `user_id`: userOid of the account used for monitoring. Normally is retrieved automatically by performing a login request. Not required.
* `refresh_token`: refresh token is used to request access token. Normally is retrieved automatically by performing a login request. Not required.
...
### `get_url` - Monitor web page state
...
* `quiet_start`: throw away new records on the first update after application startup. Default value is `false`.
* `quiet_first_time`: throw away new records produced on first update of given url. Default value is `true`.
* `store_text`: store page content in the "text" field of the record. Default value is `true`.
* `report_server_error`: emit an event if error status has been received in HTTP response, as well as when the url started responding with 2xx again. Default value is `false`.
* `report_network_error`: emit an event if a network error has occurred and when the request succeeded again after the network outage. Default value is `false`.
...
<details markdown="block">
  <summary>UrlStatusRecord</summary>

* `text`: content of the web page. Might be replaced with empty string to save space. 
* `url`: url of the monitored page. 
* `text_hash`: hash of the web page content. On network error hash of the last successfully fetched page is used. 
* `accessed_at`: time the url was fetched at. 
* `ok`: set to True if HTTP request completed successfully. 
* `status`: HTTP response status. Special value 0 means network connection failed. 
* `reason`: short text explaining status code provided by the server or error if network connection failed. 
...
### `nicochannel.news` - Monitor Nicochannel fanclub NEWS tab
...
#### Entity configuration options:
* `name`: name of a specific entity. Used to reference it in `chains` section. Must be unique within a plugin. Required.
* `url`: url of a nicochannel.jp user or a fanclub domain. Required.
##### 
* `update_interval`: how often the monitored channel will be checked, in seconds. Default value is `900`.
* `cookies_file`: path to a text file containing cookies in Netscape format. Not required.
* `headers`: custom HTTP headers as "key": value" pairs. "Set-Cookie" header will be ignored,
use `cookies_file` option instead. "Etag" and "Last-Modified" are set automatically
if available in server response. Plugin might also overwrite other headers required
...
<details markdown="block">
  <summary>NicochannelPostRecord</summary>
...
* `title`: post title. 
* `full_text`: post content. 
* `thumbnail_url`: link to the post image. 
* `published`: publication date. 
* `post_category`: post category label. 
* `post_status`: post status label. 
* `post_authorization`: post authorization label. 
* `author`: channel name. 
* `avatar_url`: link to the avatar of the channel. Not always available. 
* `fanclub_url`: link to the channel uploading the video. 
* `fanclub_id`: internal fanclub id. 

</details>

---

### `nicochannel` - Monitor Nicochannel fanclub
...
<details markdown="block">
  <summary>NicochannelVideoRecord</summary>

Nicochannel.jp video, upcoming or ongoing livestream


* `video_id`: short string identifying the video. Part of the video url. 
* `url`: video url. 
* `title`: title of the video at the time of parsing. 
* `summary`: livestream description. Not always available. 
* `published`: publication date. 
* `scheduled`: scheduled date for upcoming stream or premiere. 
* `thumbnail_url`: link to the video thumbnail. 
...
* `fanclub_url`: link to the channel uploading the video. 
* `fanclub_id`: internal fanclub id. 
* `is_upcoming`: indicates that video is an upcoming livestream. 
* `is_live`: indicates that the video is a livestream that is currently live. 
* `live_start`: . 
* `live_end`: . 
...
#### Entity configuration options:
* `name`: name of a specific entity. Used to reference it in `chains` section. Must be unique within a plugin. Required.
* `type_template`: template string with placeholders used to format "event_type" field of the produced event with corresponding values from current record. Required.
* `text_template`: template string with placeholders used to format "text" field of the produced event with corresponding values from current record. Required.
##### 
* `reset_origin`: treat throughput records as if they have originated from this entity, 
emitting them in every Chain this entity is used. Default value is `false`.
* `missing`: if specified, will be used to fill template placeholders that do not have corresponding fields in current record. Not required.
...
### `filter.format.opaque` - Generate record with custom fields
...
#### Entity configuration options:
* `name`: name of a specific entity. Used to reference it in `chains` section. Must be unique within a plugin. Required.
* `fields_templates`: mapping of fields names and corresponding template string with placeholders. Required.
##### 
* `reset_origin`: treat throughput records as if they have originated from this entity, 
emitting them in every Chain this entity is used. Default value is `false`.
* `missing`: if specified, will be used to fill template placeholders that do not have corresponding fields in current record. Not required.
...
* `transport`: HTTP transport library to use for making requests. Default value is `aiohttp`.
* `restartable`: Attempt to store unprocessed records on disk at shutdown and restart tasks to process them on the next startup. Default value is `true`.
* `url`: Twitter domain name. Default value is `https://twitter.com`.
...
* `media_url`: link to the underlying HLS stream, when available. 
* `master_url`: link to the static playlist of the underlying HLS stream, retrieved from the master playlist when medial_url is available. 
* `title`: space title. 
* `author`: user's visible name. 
* `username`: user's handle. 
* `avatar_url`: link to the picture used as the user's avatar. 
...
* `headers`: custom HTTP headers as pairs "key": value". "Set-Cookie" header will be ignored, use `cookies_file` option instead. Not required.
* `transport`: HTTP transport library to use for making requests. Default value is `aiohttp`.
* `restartable`: Attempt to store unprocessed records on disk at shutdown and process them on the next startup. Default value is `true`.
...
### `youtube.live` - Wait for livestream on Youtube
...
or a direct path to the file itself (without a slash). If special value `:memory:` is used,
database is kept in memory and not stored on disk at all, providing a clean database on every startup. Default value is `db/`.
* `consumption_delay`: delay between start of processing of multiple records received at the same time, in seconds. Used to even out short bursts of activity. Default value is `5`.
...
* `transport`: HTTP transport library to use for making requests. Default value is `aiohttp`.
* `restartable`: Attempt to store unprocessed records on disk at shutdown and restart tasks to process them on the next startup. Default value is `true`.
* `poll_interval`: how often live status of the stream is updated after the scheduled time, in seconds. Default value is `180`.
...
* `check_scheduled`: occasionally check live status before the scheduled time. Default value is `true`.
...
<details markdown="block">
  <summary>YoutubeLiveErrorEvent</summary>
...
### `nicochannel.live` - Wait for livestream on Nicochannel
...
<details markdown="block">
  <summary>NicochannelLiveErrorEvent</summary>
...
* `headers`: custom HTTP headers as pairs "key": value". "Set-Cookie" header will be ignored, use `cookies_file` option instead. Not required.
* `transport`: HTTP transport library to use for making requests. Default value is `aiohttp`.
* `restartable`: restart commands interrupted by shutdown after the next startup. Default value is `false`.
