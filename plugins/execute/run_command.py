...
class CommandEntity(ActorEntity):
    ...
    """path to directory where command will be executed. If not set current working directory is used"""
    ...
    """filename to write executed process output to. If not defined is generated automatically based on command and entity name"""
    ...
    """emit currently processed record down the chain if subprocess returned non-zero exit code. Can be used to define fallback command in case this one fails"""
    report_failed: bool = True
    """emit Event with type "error" if subprocess returned non-zero exit code or raised exception"""
    report_finished: bool = False
    """emit Event with type "finished" if subprocess returned zero as exit code"""
    report_started: bool = False
    """emit Event with type "started" before starting subprocess"""
